-- Event Ecosystem — PostgreSQL baseline schema
-- Version 1.0
-- Review migrations before production.
-- UUIDs, timestamps and money handling are intentionally explicit.
...
CREATE TYPE event_status AS ENUM ('draft','submitted','under_review','approved','published','live','completed','rejected','suspended','cancelled');
CREATE TYPE ticket_status AS ENUM ('issued','checked_in','refunded','void','cancelled','expired');
CREATE TYPE order_status AS ENUM ('created','payment_pending','paid','tickets_issued','completed','payment_failed','cancelled','refund_pending','partially_refunded','refunded');
CREATE TYPE payment_status AS ENUM ('pending','authorized','paid','failed','cancelled','refunded','partially_refunded');
CREATE TYPE refund_status AS ENUM ('requested','pending','processing','completed','failed','cancelled');
CREATE TYPE settlement_status AS ENUM ('draft','pending_review','approved','processing','paid','failed','cancelled');
CREATE TYPE checkin_result AS ENUM ('success','invalid','already_used','wrong_event','refunded','cancelled','expired','access_denied','offline_pending');
