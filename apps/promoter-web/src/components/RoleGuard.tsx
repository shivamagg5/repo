// =============================================================================
// promoter-web — RoleGuard Component
// Ensures the authenticated user has an active membership in a promoter organization.
// =============================================================================
'use client';
...
export function RoleGuard({ children }: RoleGuardProps) {
  ...
  if (status === 'loading') {
    return (
      <div className="min-h-screen bg-[#0F1117] flex flex-col items-center justify-center p-4">
        ...
        <p className="text-sm font-medium text-gray-400">Verifying promoter credentials...</p>
      </div>
    );
  }
  ...
  const hasPromoterOrg = organizations.some(
    (org) => org.type === 'promoter' && org.status === 'active',
  );

  if (!hasPromoterOrg && organizations.length === 0) {
    return (
      <div className="min-h-screen bg-[#0F1117] flex flex-col items-center justify-center p-4">
        <div className="bg-[#161922] border border-gray-800 rounded-2xl p-8 max-w-md w-full text-center shadow-2xl">
          ...
          <h2 className="text-xl font-bold text-white mb-2">Promoter Access Required</h2>
          <p className="text-sm text-gray-400 mb-6">
            You are signed in as <span className="text-gray-200 font-medium">{user?.email}</span>, but you do not belong to an active Promoter organization.
          </p>
